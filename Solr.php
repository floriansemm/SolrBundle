...
class Solr {
	 ...
	 */
	public function __construct(Client $client, CommandFactory $commandFactory, EventDispatcherInterface $manager, MetaInformationFactory $metaInformationFactory, EntityMapper $entityMapper) {
	...
	public function createQuery($entity) {
		...
		$query = new SolrQuery();
		$query->setSolr($this);
		$query->setEntityMetaInformation($metaInformation);
	...
	public function getRepository($entityAlias) {
		...
		if (class_exists($repositoryClass)) {
			...
			throw new \RuntimeException(sprintf(
					'%s must extends the FS\SolrBundle\Repository\Repository', $repositoryClass
			));
		}
	...
	public function query(AbstractQuery $query) {
		...
		foreach ($response as $document) {
			$mappedEntities[] = $this->entityMapper->toEntity($document, $query->getEntityMetaInformation());
		}
